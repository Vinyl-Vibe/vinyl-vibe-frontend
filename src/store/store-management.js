...
// Helper to format money values - Modified to return numbers instead of strings
const formatMoney = (amount) => {
    // Convert to number if it's a string
    const num = Number(amount);
    // Return 0 if not a valid number
    if (isNaN(num)) return 0;
    // Return the number with 2 decimal places if needed
...
export const useStoreManagement = create((set, get) => ({
    ...
    calculateStats: (orders, users, carts) => {
        ...
        // Calculate abandoned carts and values (now returning numbers)
        ...
        const totalAbandonedValue = formatMoney(
            abandonedCarts.reduce((sum, cart) => {
                return (
                    sum +
                    cart.products.reduce((cartSum, item) => {
        ...
        const todayAbandonedValue = formatMoney(
            abandonedCarts
        ...
        // Calculate total sales (now returning numbers)
        const totalSales = formatMoney(
            completedOrders.reduce((sum, order) => sum + order.total, 0),
        );
        const totalSalesToday = formatMoney(
    ...
    updateProduct: async (productId, productData) => {
        set({ isLoading: true, error: null });
        try {
            const response = await storeManagementApi.updateProduct(
                productId,
                productData,
            );
    ...
    createProduct: async (productData) => {
        ...
        try {
            const response =
                await storeManagementApi.createProduct(productData);
    ...
    updateOrderStatus: async (orderId, status) => {
        try {
            const response = await storeManagementApi.updateOrderStatus(
                orderId,
                status,
            );
