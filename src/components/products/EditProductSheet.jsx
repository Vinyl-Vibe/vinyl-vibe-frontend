...
export default function EditProductSheet({ product, open, onOpenChange, isCreating }) {
    ...
    useEffect(() => {
        if (isCreating) {
            ...
            const isChanged = 
                String(formData.name || '') !== String(currentProduct.name || '') ||
                Number(formData.price || 0) !== Number(currentProduct.price || 0) ||
                String(formData.type || '') !== String(currentProduct.type || '') ||
                String(formData.description || '') !== String(currentProduct.description || '') ||
                Number(formData.stock || 0) !== Number(currentProduct.stock || 0) ||
                String(formData.brand || '') !== String(currentProduct.brand || '') ||
                String(formData.albumInfo?.artist || '') !== String(currentProduct.albumInfo?.artist || '') ||
                String(formData.albumInfo?.genre || '') !== String(currentProduct.albumInfo?.genre || '') ||
                String(formData.albumInfo?.releaseDate || '') !== String(currentProduct.albumInfo?.releaseDate || '') ||
                JSON.stringify(formData.albumInfo?.trackList || []) !== JSON.stringify(currentProduct.albumInfo?.trackList || [])
    ...
    // Add new track
    const addTrack = () => {
        handleAlbumInfoChange('trackList', [...formData.albumInfo.trackList, ''])
    }

    // Remove track
    const removeTrack = (index) => {
        const newTrackList = formData.albumInfo.trackList.filter((_, i) => i !== index)
        handleAlbumInfoChange('trackList', newTrackList)
    }
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (result.success) {
                setStatus('success')
                setMessage(`Product ${isCreating ? 'created' : 'updated'} successfully`)
            } else {
                console.error("Failed to update product:", result.error)
                setStatus('error')
                setMessage(result.error || `Failed to ${isCreating ? 'create' : 'update'} product`)
            }
        } catch (error) {
        ...
        <Sheet open={open} onOpenChange={handleSheetClose}>
            <SheetContent className="sm:max-w-[540px]">
                {(currentProduct || isCreating) ? (
                    <>
                        <SheetHeader>
                            <SheetTitle>{isCreating ? 'Add Product' : 'Edit Product'}</SheetTitle>
                            <SheetDescription>
                                {isCreating 
                            ...
                            {formData.type === 'vinyl' && (
                                <>
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            id="artist"
                                            value={formData.albumInfo.artist}
                                            onChange={(e) => handleAlbumInfoChange('artist', e.target.value)}
                                        />
                                    </div>
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => handleAlbumInfoChange('genre', e.target.value)}
                                        />
                                    </div>
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            id="releaseDate"
                                            type="date"
                                            value={formData.albumInfo.releaseDate ? format(new Date(formData.albumInfo.releaseDate), 'yyyy-MM-dd') : ''}
                                            onChange={(e) => handleAlbumInfoChange('releaseDate', e.target.value)}
                                        />
                                    </div>

                                    <div className="grid gap-2">
                                        <label>Track List</label>
                                        {formData.albumInfo.trackList.map((track, index) => (
                                            <div key={index} className="flex gap-2">
                                                <Input
                                                    value={track}
                                                    onChange={(e) => handleTrackListChange(index, e.target.value)}
